...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-custom-modal-admin runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-custom-modal-admin.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ custom_modal_admin")
...
def lint(c):
    ...
    """
    c.run("flake8 django-custom-modal-admin tests")
...
def release(c, bumpsize=''):
    ...
    import django_custom_modal_admin
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_custom_modal_admin.__version__))
    c.run("git push --tags")
